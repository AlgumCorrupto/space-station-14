...
namespace Content.Client.Dmm
{
    public sealed class DmmComverter
    {
        ...
        [Dependency] private readonly ISerializationContext _ctx = default!;
        ...
        private IEnumerable<ITileDefinition> _tileDefs = default!;
        public DmmComverter()
        {
        ...
        private MappingDataNode CreateChunk(int[,] tileIDs, int xOffs, int yOffs)
        {
            ...
            {
                {"ind", String.Format("{0},{1}", xOffs, yOffs)},
        ...
        MappingDataNode GenerateMapProto()
        {
            var proto = new MappingDataNode();
            var component = new MappingDataNode();
            var entities = new MappingDataNode()
            {
                { "uid", "1" },
                { "components", component }
            };
            proto.Add("entities", entities);

            component.Add("type", "Broadphase");
            component.Add("type", "OccluderTree");
            component.Add("type", "Shuttle");
            component.Add("type", "GridPathfinding");
